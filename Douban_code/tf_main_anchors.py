import os
from tf_model import Model_Anchors
from tf_trainer_anchors import Trainer
...
def train():
    ...
    for random_seed in [2019]:
        ...
        # Model Parameters
        meta_layer_sizes = [256, sum(layer_sizes), sum(layer_sizes)]
        meta_layer_acts = ['tanh', 'tanh', 'sigmoid']
        meta_layer_keeps = [1.0, 1.0, 1.0]
        # meta_layer_keeps = [1.0, 1.0, 0.8]
        fuse = 3
        head_num = 2
        anchor_num = 5
        anchor_dim = 64
        ...
        print('source batch size:', DATA_CONFIG['TRAIN_SRC']['batch_size'], 'target batch size:',
              DATA_CONFIG['TRAIN_TRG']['batch_size'])
        ...
        print("test ndcg, hr", "name:", name, "dc:", dc, "random_seed:", random_seed, "l1_w:", l1_w, "save:", save,
              "drop_epoch:", drop_epoch)
        model = Model_Anchors(init="xavier", user_max_id=src_dataset.feat_sizes[0],
                                      ...
                                      meta_layer_sizes=meta_layer_sizes, meta_layer_acts=meta_layer_acts,
                                      meta_layer_keeps=meta_layer_keeps, fuse=fuse, anchor_num=anchor_num,
                                      anchor_dim=anchor_dim, cotrain=cotrain, orth=orth, orth_alpha=orth_alpha,
                                      head_num=head_num, meta_share=meta_share, use_meta_base=use_meta_base,
                                      seperate_loss=seperate_loss)
